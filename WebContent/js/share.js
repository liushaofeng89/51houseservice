/**
 * script from http://www.xunhuweb.com/blog/416
 */
...
function shareToSinaWB(event) {
	...
	var _shareUrl = 'http://v.t.sina.com.cn/share/share.php?&appkey=4023998718'; // 真实的appkey，必选参数
	_shareUrl += '&url=' + encodeURIComponent(_url || document.location); // 参数url设置分享的内容链接|默认当前页location，可选参数
	_shareUrl += '&title=' + encodeURIComponent(_title || document.title); // 参数title设置分享的标题|默认当前页标题，可选参数
	_shareUrl += '&source=' + encodeURIComponent(_source || '');
	_shareUrl += '&sourceUrl=' + encodeURIComponent(_sourceUrl || _url);
...
function shareToQzone(event) {
	...
	_shareUrl += '&site=' + encodeURIComponent(_site || _url); // 参数site设置分享来源，可选参数
...
function shareToQQwb(event) {
	...
	_shareUrl += '&pic=' + encodeURIComponent(_pic || ''); // 分享的图片，如果是多张图片，则定义var
															// _pic='图片url1|图片url2|图片url3....'
	window
			.open(
					_shareUrl,
					'_blank',
